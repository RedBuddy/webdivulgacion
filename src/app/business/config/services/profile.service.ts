...
export class ProfileService {
  ...
  getProfile(): Observable<Profile> {
    ...
    return this.http.get<Profile>(`${this.profileUrl}/${userId}`).pipe(
      catchError(error => {
        console.log('WEBOS AQUI ESTA EL PEDO:', error);
        if (error.status === 404) {
          // Si el perfil no existe, crear uno nuevo
          const newProfile: Profile = { id_user: userId, biography: '', experience: '' };
          return this.createProfile(newProfile).pipe(
            switchMap(() => this.getProfile()) // Obtener el perfil recién creado
          );
        } else {
